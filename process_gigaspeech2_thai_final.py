...
Final cleaned implementation with correct data structure
"""
...
print("GigaSpeech2 Thai Processing with Pattern→MetricGAN+")
print("=" * 80)
...
def load_gigaspeech2_samples(num_samples=50, filter_thai=True):
    """Load GigaSpeech2 samples, optionally filtering for Thai"""
    print(f"\nLoading {num_samples} samples from GigaSpeech2...")
    ...
    try:
        ...
        for item in dataset:
            ...
            try:
                # Extract audio from correct field
                wav_info = item['wav']
                audio_array = wav_info['array']
                sample_rate = wav_info['sampling_rate']
                
                # Get ID and transcript if available
                sample_id = item.get('__key__', f'gs2_{sample_count+1:04d}')
                
                # If filtering for Thai, check the ID pattern or other metadata
                # GigaSpeech2 Thai samples often have specific patterns
                if filter_thai:
                    # Simple heuristic - adjust based on actual data structure
                    # You may need to check transcript language or other fields
                    pass
                ...
                if sample_count % 10 == 0:
                    print(f"  Loaded {sample_count}/{num_samples} samples...")
...
def main():
    ...
    if processing_times:
        ...
        # Save report
        report_path = output_dir / "gigaspeech2_50_samples_final_report.md"
        with open(report_path, 'w') as f:
            f.write("# GigaSpeech2 - Pattern→MetricGAN+ Processing Report\n\n")
            f.write(f"## Summary\n")
            f.write(f"- Dataset: GigaSpeech2\n")
            f.write(f"- Samples processed: {successful}/{len(samples)}\n")
            f.write(f"- Method: Pattern→MetricGAN+ (Final Winner)\n\n")
            
            f.write(f"## Performance\n")
            f.write(f"- Average time: {avg_time:.2f}s per sample\n")
            f.write(f"- Processing rate: {len(samples)/total_time:.1f} samples/sec\n")
            f.write(f"- Estimated 10M: {(avg_time * 10_000_000 / 3600):.1f} hours\n\n")
            
            f.write(f"## Results\n")
            f.write(f"- Total interruptions: {total_interruptions}\n")
